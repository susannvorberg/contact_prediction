...
##################################################################################################################
#
# Run PERSISTENT contrastive divergence with standard settings:
...
#bash ~/opt/contactprediction/contact_prediction/run/full_likelihood_optimization/run_pcd.sh 5
...
mat_dir="/usr/users/svorber/work/data/benchmark_contrastive_divergence/phd/pcd/"
...
do
    if [ ! -d "$mat_dir/$step" ]; then
        mkdir -p $mat_dir/$step
    fi

    find $mat_dir/$step/*log -size 0 -delete
done
...
do
    ...
    for step in $steps_set;
    do
        matfile=$mat_dir/$step/$name.mat
        logfile=$mat_dir/$step/$name.log
        ...
        then
            ...
            settings=$settings" --ofn-cd  --cd-gibbs_steps $step --cd-sample_size 0.3 --cd-sample_ref Neff --cd-persistent"
            settings=$settings" --alg-gd --alpha0 0"
            settings=$settings" --decay --decay-start 1e-1 --decay-rate 5e-6 --decay-type sig"
            settings=$settings" --early-stopping --epsilon 1"
            ...
            echo "start PCD at eps: 1"
            echo "log file: "$logfile
            echo $settings
            echo "--------------------------------------------------"
            echo " "

            jobname=ccmpredpy_pcd.gibbsstep$step.$name
